...
int tftp_main(int argc UNUSED_PARAM, char **argv)
{
  ...
  IF_GETPUT(opt =)
  getopt32(argv,
           IF_FEATURE_TFTP_GET("g") IF_FEATURE_TFTP_PUT("p") "l:r:" IF_FEATURE_TFTP_BLOCKSIZE("b:"),
           &local_file, &remote_file IF_FEATURE_TFTP_BLOCKSIZE(, &blksize_str));
  argv += optind;
  ...
  result = tftp_protocol(
      ...
      local_file, remote_file IF_FEATURE_TFTP_BLOCKSIZE(, 1 /* want_transfer_size */) IF_FEATURE_TFTP_BLOCKSIZE(, blksize));
  tftp_progress_done();
...
int tftpd_main(int argc UNUSED_PARAM, char **argv)
{
  ...
  if (result < 4 || result >= sizeof(block_buf) || block_buf[result - 1] != '\0' || (IF_FEATURE_TFTP_PUT(opcode != TFTP_RRQ)               /* not download */
                                                                                     IF_GETPUT(&&) IF_FEATURE_TFTP_GET(opcode != TFTP_WRQ) /* not upload */
                                                                                     ))
  {
    goto err;
  }

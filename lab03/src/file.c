...
void get_list_users_from_file()
{
  ...
  while (1)
  {
    ...
    if (i != 4) // must be username password status
    {
    ...
    User *user = create_new_user(words[0], words[1], strtol(words[2], NULL, 10), words[3]);
    if (user == NULL)
    {
...
void update_list_users_to_file()
{
  ...
  while (tmp)
  {
    sprintf(user, "%s %s %d %s\n", tmp->user->username, tmp->user->password, tmp->user->status, tmp->user->homepage);

...
#define FILE_PATH "nguoidung.txt"
...
void signin()
{
  ...
  while (1)
  {
    ...
    if (response == SUCCESS)
    {
      printf("%s\n", set_color(GREEN, "Hello %s", username));
      break;
    }
    else
    {
...
void homepage_with_domain()
{
  ...
  if (is_valid_domain(user->homepage) == 1)
  {
    printf("%s\n", set_color(GREEN, "Homepage: %s", user->homepage));
  }
  else
  {
    char domain[1024];
    char *result = ipaddr_to_domain(user->homepage, domain);
    if (result == NULL)
    {
    ...
    {
      printf("%s\n", set_color(GREEN, "Homepage: %s", result));
    }
  }
}
...
void homepage_with_ipaddr()
{
  ...
  if (is_valid_ipaddr(user->homepage) != 0)
  {
    printf("%s\n", set_color(GREEN, "Homepage: %s", user->homepage));
  }
  else
  {
    char ipaddr[1024];
    char *result = domain_to_ipaddr(user->homepage, ipaddr);
    if (result == NULL)
    {
      error(ERR_IPADDR_NOT_FOUND);
    }
    else
    {
      printf("%s\n", set_color(GREEN, "Homepage: %s", result));
